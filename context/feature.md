...
- [x] **Sessions & devices** — `GET /api/sessions` (device label + IP + `current` flag), `DELETE /api/sessions/:id` (log out a device), `POST /api/sessions/revoke-others` (log out other devices); over the `Session` model, current session matched by refresh-cookie hash. _Multi-account switching deferred to v2._
