...
25. **Frontend connection** (both repos): backend already CORS-locks to `CLIENT_ORIGIN` with `credentials:true`; frontend uses `VITE_API_URL`, `credentials:'include'`, echoes the `ms_csrf` cookie as `x-csrf-token` on mutations, and refreshes on 401. Prod: sibling subdomains under one parent + `COOKIE_DOMAIN=.domain`. No code-level link — the contract is docs/SPEC.md + the route list in summary.md.
