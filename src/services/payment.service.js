...
export async function createPayoutOrder({ merchantOrderId, amount, account, name, bnkCode, ip, remark }) {
  ...
  if (raw?.code !== 200) {
    // Full payload (no secret — `sign` is added inside post()) + gateway reply,
    // so a rejection like 该ip禁止访问 is diagnosable field-by-field.
    logger.warn({ payload, raw }, 'Gateway payout rejected');
  }
