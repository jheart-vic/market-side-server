...
- **Sessions & devices** (`/api/sessions`, requireAuth): `GET /` (active sessions — parsed device label via `utils/userAgent.js`, IP, timestamps, `current` flag matched by refresh-cookie hash), `DELETE /:id` (log out a device), `POST /revoke-others` (log out all other devices). Over the existing `Session` model (no schema change); `session.service.js`. Revoke kills the refresh token immediately; that device's access token lapses within ACCESS_TOKEN_TTL. **Multi-account switching deferred to v2**
