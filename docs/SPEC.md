...
- **Release window is admin-configurable** (settings `signal_release_start`/`signal_release_end`, Lagos wall-clock, default **15:00–17:00**; set 00:00–23:59 for any-time). The window governs **only auto-release**: a signal **created inside the window goes live immediately**; otherwise it stays scheduled until the job publishes it when the window opens, or an admin `force`-releases it early. **Released = tradeable** — once a signal is released (auto or forced), users can place contracts on it right away regardless of the clock; it stays tradeable through its release day.
